...
- **Driver POD Upload Portal**: Public (no login required), mobile-optimized web app at `/driver-pod` for drivers to upload Proof of Delivery documents. Drivers enter their name, load number, and truck number, then attach photos or PDFs. Uses the public endpoint POST `/api/public/pod-upload` which looks up the load by number, appends POD files, and auto-marks the load as delivered. Camera capture and multi-file support included. After a successful upload, a dispatch notification email is sent automatically if `dispatchNotificationEmail` is configured in Company Settings. Includes a **"Check Submission Status"** section (no login required) backed by `GET /api/public/pod-status/:loadNumber` — drivers can look up any load number to see how many PODs have been submitted, when, and by whom.
...
- **Feedback**: User feedback submission module accessible to all roles. Captures person name, free-form note, and optional file attachment (image or PDF, up to 10MB). All submissions are stored in the `feedbacks` table. **Admin-only restrictions**: The submitted feedback list (`GET /api/feedbacks`) is restricted to admin users; non-admin (dispatch) users can only submit feedback and do not see the list. Each feedback entry has an `open`/`resolved` status badge; admins can toggle it via `PATCH /api/feedbacks/:id/status` and the "Mark Resolved" / "Mark Open" button in the UI. Admins can also delete entries. Sidebar item appears between "Recurring Expenses" and "Tasks".
