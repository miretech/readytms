...
- **Driver Settlements**: Automated payroll calculations with percentage-based pay, deduction tracking, net pay, and settlement history with PDF export. **Settlement Calculation Flow (Fixed Oct 2025)**: Factoring fees are deducted from Total Revenue to get Net Revenue, then driver pay is calculated as a percentage of Net Revenue (not gross revenue). The `settlement.deductions` field stores ONLY other deductions (tolls, fuel, advance, insurance, trailer fees, repairs) and does NOT include factoring fee. Net Pay = Driver Pay - Other Deductions. This prevents factoring from being deducted twice and ensures consistency between UI, database, and PDF exports.
